 ...
 * В данном случае выводяться значения "Recording << " и "Stop Recording >> " неверно, потому что как таковой нет основной
 * мысли и специфики как это реализовать.
 * Так же в коде есть некоторые закомментированные строки - это остатки от бывших проб и выриаций.
